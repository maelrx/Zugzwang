# Zugzwang Research Kernel: Master Technical Design

**Status:** baseline arquitetural greenfield, pre-scaffold  
**Edição integrada:** 12 de agosto de 2026  
**Fonte-base:** technical design de 11 de agosto de 2026, preservado em `archive/source-material/`  
**Normatividade:** ADR aceita > protocol spec > requirement > este documento > material arquivado

## Atualização integrada desta edição

O desenho-base permanece válido, com as seguintes extensões normativas:

1. **Multimodalidade no contrato, não na UI.** `ModelRequest` recebe content parts tipados e imagens são artifacts CAS imutáveis.
2. **Dois eixos de assistência.** `H0-H7` mede assistência operacional/estratégica e `K0-K7` mede conhecimento externo.
3. **KnowledgePacket no v0.1.** Conhecimento estático versionado entra sem vector database e sem retrieval online.
4. **RAG continua adiado.** `K6` position-conditioned retrieval é um regime posterior e precisa de provenance própria.
5. **Experiment program v0.1.** REP-001, GROUND-001, SKILL-001, MM-001, MM-002 e DEMO-001 definem o primeiro uso científico.
6. **Python virou gate explícito.** A recomendação é floor 3.13 com CI 3.13/3.14, mas o operador ratifica.
7. **SQLite runtime check.** WAL requer SQLite corrigido contra o WAL-reset bug: 3.51.3+, 3.53.x corrente ou backport oficial conhecido.
8. **Licença permanece gate.** O arquivo `LICENSE` não existe até a decisão rules substrate/licença.
9. **Retention é política.** Raw evidence pode ser criptografada, redacted ou omitida por restrição, mas a política precisa aparecer no bundle.
10. **Codex execution contract.** `AGENTS.md` aninhados e `.agents/skills` são parte da engenharia do repositório, não parte do runtime científico.

## Decisões novas relacionadas

- ADR-033: typed multimodal content parts;
- ADR-034: dual assistance axes H/K;
- ADR-035: static KnowledgePacket before RAG;
- ADR-036: evidence retention policy;
- ADR-037: CLI naming;
- ADR-038: documentation language;
- ADR-039: Stockfish acquisition;
- ADR-040: provider output redistribution;
- ADR-041: cost registry governance;
- ADR-042: standard chess versus variants;
- ADR-043: plugin API stability;
- ADR-044: SQLite minimum runtime.
