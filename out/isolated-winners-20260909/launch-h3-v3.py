...
assert not (out/'launch-h3-v3.json').exists(),'Already launched; never duplicate runs'
rows=json.loads((out/'selection-h3.json').read_text());procs=[]
...
for row in rows:
 ...
 log=(out/f'run-{row["index"]:02d}-v3.log').open('ab');cmd=[str(r/'.venv/bin/zugzwang'),'run',row['manifest'],'--workspace',str(ws),'--output','json'];env=os.environ.copy();env['PYTHONUNBUFFERED']='1'
 ...
 (out/'launch-h3-v3.json').write_text(json.dumps(procs,indent=2))
