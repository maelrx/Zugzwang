...
assert not (out/'launch-v2.json').exists(),'Already launched; never duplicate runs'
...
for row in rows:
 ...
 log=(out/f'run-{row["index"]:02d}-v2.log').open('ab');cmd=[str(r/'.venv/bin/zugzwang'),'run',row['manifest'],'--workspace',str(ws),'--output','json'];env=os.environ.copy();env['PYTHONUNBUFFERED']='1'
 ...
 (out/'launch-v2.json').write_text(json.dumps(procs,indent=2))
