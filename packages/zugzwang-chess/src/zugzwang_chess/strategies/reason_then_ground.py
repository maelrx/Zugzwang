...
from ._prompt_hooks import apply_prompt_override, apply_prompt_override_text
...
class ReasonThenGroundStrategy:
    ...
    async def decide(self, observation: Any, context: DecisionContext) -> DecisionTrace:
        ...
        try:
            analysis, candidates, phase1_record = await self._phase1(obs, context)
        ...
        phase2 = await self._phase2(obs, legal_actions, analysis, context)
        return DecisionTrace(
            strategy_id=self.strategy_id,
            strategy_version=self.strategy_version,
            declared_regime="R1",
