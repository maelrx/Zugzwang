...
const historyRoute = (hash: string) => hash === "#historico" ? "" : hash.startsWith("#historico/") ? hash.slice(11) : null;
...
export default function App() {
  ...
  const [historyId, setHistoryId] = useState(() => historyRoute(window.location.hash));
  const [play, setPlay] = useState(() => window.location.hash === "#jogar");
  useEffect(() => { const read = () => { setRoute(parseRoute(window.location.hash)); setHistoryId(historyRoute(window.location.hash)); setComparing(window.location.hash === "#comparar"); setPlay(window.location.hash === "#jogar"); }; window.addEventListener("hashchange", read); return () => window.removeEventListener("hashchange", read); }, []);
  ...
  const back = () => { window.location.hash = ""; setRoute(parseRoute("")); setComparing(false); setPlay(false); setHistoryId(null); };
  const goPlay = () => { window.location.hash = "jogar"; setComparing(false); setPlay(true); setHistoryId(null); };
  const goHistory = () => { window.location.hash = "historico"; setHistoryId(""); setPlay(false); setComparing(false); };
  const arenaMode = play || historyId !== null;
  ...
  const textureStyle = texture.uri
    ? {
        ...
        opacity: appearance.theme === "light" ? texture.light.opacity : Math.min(texture.dark.opacity, 0.025),
      }
    : undefined;
  return <>
    {textureStyle && <div className="texture-overlay" style={textureStyle} aria-hidden="true" />}
    <div className={`app-shell${arenaMode ? " arena-shell" : ""}`}>
    ...
    <aside className="sidebar">
      ...
      <div className="workspace-label"><span className="workspace-avatar">L</span><div><strong>Workspace local</strong><span>{arenaMode ? "Partidas pessoais" : "Somente leitura"}</span></div></div>
      <nav className="sidebar-nav" aria-label="Navegação principal">
        <button className={play ? "active" : ""} aria-current={play ? "page" : undefined} onClick={goPlay}><Icon name="play"/>Jogar</button>
        <button className={historyId !== null ? "active" : ""} aria-current={historyId !== null ? "page" : undefined} onClick={goHistory}><Icon name="clock"/>Histórico</button>
        <button className={historyId === null && !play && !comparing && (Boolean(route.runId) || status !== "attention") ? "active" : ""} onClick={() => filter("all")}><Icon name="grid"/>Partidas<span className="nav-count">{runs.length}</span></button>
        <button className={historyId === null && !play && !comparing && !route.runId && status === "attention" ? "active" : ""} onClick={() => filter("attention")}><Icon name="alert"/>Atenção<span className="nav-count">{runs.filter(r => runState(r).key === "attention").length}</span></button>
        <button className={comparing ? "active" : ""} onClick={() => { window.location.hash = "comparar"; setPlay(false); setHistoryId(null); setComparing(true); }}><Icon name="compare"/>Comparar{selectedRuns.length > 0 && <span className="nav-count">{selectedRuns.length}</span>}</button>
      </nav>
      ...
      <div className="sidebar-footer"><span className="local-dot"/>Dados locais e privados<p>{arenaMode ? "Partidas privadas contra os modelos do workspace." : "Esta interface não inicia nem modifica execuções."}</p></div>
    </aside>
    <div className="workspace-main"><header className="topbar"><div className="breadcrumb"><button onClick={back}>Workspace</button><Icon name="chevron" size={13}/><span>{historyId !== null ? "Histórico" : play ? "Jogar" : comparing ? "Comparação" : route.runId ? "Execução" : "Biblioteca"}</span></div><div className="freshness" title={Number.isFinite(date) ? `Dados gerados em ${new Date(date).toLocaleString("pt-BR")}` : "Horário de geração não informado"}><i className={error || stale ? "warning" : ""}/><span>{arenaMode ? "Local e privado" : error ? "Atualização indisponível" : Number.isFinite(date) ? `Dados ${ageLabel(date)}` : "Aguardando dados"}</span></div><AppearanceMenu/>{!arenaMode && <button className="button compact" onClick={() => void reload()} disabled={refreshing} aria-label="Atualizar dados"><Icon name="refresh" size={15}/><span>{refreshing ? "Atualizando" : "Atualizar"}</span></button>}</header>
      <main id="main-content" className="main-content" tabIndex={-1}>
        {error && !arenaMode && <div className="notice error" role="alert"><Icon name="alert"/><div><strong>{snap ? "A última leitura continua disponível" : "Não foi possível carregar as execuções"}</strong><p>{snap ? "A atualização falhou. Você pode continuar a análise e tentar novamente." : "Verifique se a fonte de dados do workspace está disponível e tente atualizar."}</p><details><summary>Detalhes de conexão</summary><code>{error} · /data/data.json</code></details></div><button className="button" onClick={() => void reload()}>Tentar novamente</button></div>}
        {loading && !snap && !arenaMode ? <LoadingState/> : historyId !== null ? <Suspense fallback={<LoadingState/>}><HistoryView key={historyId} gameId={historyId}/></Suspense> : play ? <Suspense fallback={<LoadingState/>}><PlayView/></Suspense> : snap && (comparing ? <CompareView runs={selectedRuns} onBack={back}/> : route.runId ? current ? <>
      ...
      </main><footer className="workspace-footer"><span>Zugzwang · pesquisa verificável</span><span>{arenaMode ? "Partidas salvas neste computador" : "Leitura automática a cada 15 s"}</span></footer>
    </div>
    </div>
  </>;
}
