...
const CognitiveView = lazy(() => import("@/components/CognitiveView").then(m => ({ default: m.CognitiveView })));
...
export default function App() {
  ...
  const [cognitive, setCognitive] = useState(() => window.location.hash === "#cognitivo");
  const [play, setPlay] = useState(() => window.location.hash === "#jogar");
  useEffect(() => { const read = () => { setRoute(parseRoute(window.location.hash)); setComparing(false); setCognitive(window.location.hash === "#cognitivo"); setPlay(window.location.hash === "#jogar"); }; window.addEventListener("hashchange", read); return () => window.removeEventListener("hashchange", read); }, []);
  ...
  const back = () => { window.location.hash = ""; setRoute(parseRoute("")); setComparing(false); setCognitive(false); setPlay(false); };
  const goCognitive = () => { window.location.hash = "cognitivo"; setComparing(false); setCognitive(true); setPlay(false); };
  const goPlay = () => { window.location.hash = "jogar"; setComparing(false); setCognitive(false); setPlay(true); };
  ...
  return <>
    ...
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav" aria-label="Navegação principal">
        <a className={cognitive ? "active" : ""} href="#cognitivo" onClick={(e) => { e.preventDefault(); goCognitive(); }}><Icon name="grid"/>Cognitivo</a>
        <a className={play ? "active" : ""} href="#jogar" onClick={(e) => { e.preventDefault(); goPlay(); }}><Icon name="play"/>Jogar</a>
    ...
    <div className="workspace-main"><header className="topbar"><div className="breadcrumb"><button onClick={back}>Workspace</button><Icon name="chevron" size={13}/><span>{comparing ? "Comparação" : route.runId ? "Execução" : "Biblioteca"}</span></div><div className="freshness" title={Number.isFinite(date) ? `Dados gerados em ${new Date(date).toLocaleString("pt-BR")}` : "Horário de geração não informado"}><i className={error || stale ? "warning" : ""}/><span>{error ? "Atualização indisponível" : Number.isFinite(date) ? `Dados ${ageLabel(date)}` : "Aguardando dados"}</span></div><AppearanceMenu/><button className="button compact" onClick={() => void reload()} disabled={refreshing} aria-label="Atualizar dados"><Icon name="refresh" size={15}/><span>{refreshing ? "Atualizando" : "Atualizar"}</span></button></header>
      <main id="main-content" className="main-content" tabIndex={-1}>
        ...
        {loading && !snap && !cognitive && !play ? <LoadingState/> : play ? <Suspense fallback={<LoadingState/>}><PlayView/></Suspense> : cognitive ? <Suspense fallback={<LoadingState/>}><CognitiveView/></Suspense> : snap && (comparing ? <CompareView runs={selectedRuns} onBack={back}/> : route.runId ? current ? <>
